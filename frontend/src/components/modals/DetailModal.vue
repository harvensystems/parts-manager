<template>
  ...
  >
    <div v-if="store.selectedComponent" class="space-y-4 text-xs sm:text-sm">
      ...
      <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 dark:border-slate-800 pb-3">
        <div class="flex items-center gap-2">
          ...
          <span 
            ...
            :class="store.selectedComponent.mounting === 'SMD' ? 'bg-indigo-100 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300 border border-indigo-300 dark:border-indigo-700/40' : 'bg-amber-100 dark:bg-amber-950 text-amber-700 dark:text-amber-300 border border-amber-300 dark:border-amber-700/40'"
          >
            {{ store.selectedComponent.mounting }}
          </span>
          <span v-if="store.selectedComponent.package" class="px-2 py-0.5 rounded text-xs font-mono bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-transparent">
        ...
        <div class="flex items-center space-x-2 bg-slate-100 dark:bg-slate-950 p-1 rounded-lg border border-slate-200 dark:border-slate-800">
          ...
          <span class="px-2 font-mono font-bold text-emerald-600 dark:text-emerald-400 text-sm">{{ store.selectedComponent.quantity }} {{ t('detailModal.pcs') }}</span>
          <button 
            @click="store.adjustQuantity(store.selectedComponent, 1)" 
            class="w-7 h-7 flex items-center justify-center rounded hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white text-sm cursor-pointer"
      ...
      <div>
        <h2 class="text-base sm:text-lg font-bold text-slate-900 dark:text-white">{{ store.selectedComponent.name }}</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400 font-mono mt-0.5">
          {{ t('detailModal.manufacturer') }} <strong class="text-slate-700 dark:text-slate-200">{{ store.selectedComponent.manufacturer || t('detailModal.notSpecified') }}</strong>
        </p>
      </div>

      <!-- Image Display -->
      <div v-if="store.selectedComponent.photo" class="rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 h-48 sm:h-56">
